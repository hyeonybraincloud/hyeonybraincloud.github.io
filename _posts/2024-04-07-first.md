...
deep triode region은 앞서 보인 $i_D-v-{DS}$ 그래프에서 원점에 가까운 부분을 가리킨다. **$v_{GS}>V_{Th,n}>0$**을 Gate에 인가하여 채널이 형성된 상태에서 Drain-Source 간에 $v_{DS}>0$을 인가하면 채널 내의 electrons이 전계에 이끌려 Drain으로 이동하므로 그 역방향(Drain to Source)으로 $i_D$가 흐르게 된다. 이 영역에서는 $i_D$가 다음 식과 같은 관계를 보인다.
