...
$v_{DS}$가 특정한 값 이상으로 증가하면 Drain 주변의 공핍층이 확장되어 채널 크기가 감소되어 더 이상 $v_{DS}$가 증가하는 만큼 $i_D$가 커지지 않게 된다. 그런데도 $v_{DS}$가 증가하면 채널이 소멸되는 **채널 핀치-오프(pinch-off)**가 발생한다. 이때의 $v_{DS}$을 $v_{DS(sat)}$라고 정의한다. 앞서 언급된 특성 곡선 그림에서 볼 수 있듯이, **채널이 소멸된 핀치-오프 이후**에도 **$i_D\neq 0$**이고, 일정한 크기로 **포화된 전류 특성을 가지는 포화 영역에서 동작**하게 된다. 이때 기판 쪽으로 공핍층이 형성되게 하기 위해 Drain과 Source의 불순물을 더 강하게 $N^+$ 도핑시킨다.
